...
class GymRouter {
    ...
    updateActiveTab(activeRoute) {
        ...
        if (activeLink) {
            ...
            activeLink.classList.remove("hover:bg-purple-700/50", "dark:hover:bg-gray-700/50", "transition-colors");
        }
    }
    ...
    async showLoginPage() {
        ...
        const mainContent = document.querySelector('.ml-12.sm\\:ml-48') || document.body;
        ...
        try {
            ...
            mainContent.innerHTML = '<div class="flex items-center justify-center min-h-screen"><div class="text-red-500">Error loading login page.</div></div>';
        }
    }

    setupLogoutButton() {
        // Find logout button and add click handler
        const logoutBtn = document.querySelector('button:has(.fa-arrow-right-from-bracket)');
        if (logoutBtn) {
    ...
    async handleInitialRoute() {
        ...
        const initialRoute = hash && this.routes[hash] && hash !== "login" ? hash : "overview";
