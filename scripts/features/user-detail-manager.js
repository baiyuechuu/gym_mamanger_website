...
class UserDetailManager {
    ...
    setupEventListeners() {
        ...
        editPackageSelect?.addEventListener('change', () => this.calculateEndDate());
        editStartDateInput?.addEventListener('change', () => this.calculateEndDate());
        ...
        const closeTerminationBtn = document.getElementById('close-termination-modal-btn');
        const cancelTerminationBtn = document.getElementById('cancel-termination-btn');
        closeTerminationBtn?.addEventListener('click', () => this.closeTerminationModal());
        cancelTerminationBtn?.addEventListener('click', () => this.closeTerminationModal());

        // Termination form submission
        const terminationForm = document.getElementById('termination-form');
        terminationForm?.addEventListener('submit', (e) => this.handleTerminationSubmit(e));
    ...
    openEditModal() {
        ...
        document.getElementById('edit-package-type').value = this.currentUser.packageType || '';
        document.getElementById('edit-start-date').value = this.currentUser.startDate || '';
        document.getElementById('edit-end-date').value = this.currentUser.endDate || '';
    ...
    async handleEditSubmit(e) {
        ...
        try {
            ...
            submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Saving Changes...';
            ...
            // Update member data
            const updatedMember = this.dataManager.updateMember(this.currentUser.bookingNo, updatedData);
            ...
            this.showMessage('Failed to update member information. Please try again.', 'error');
        } finally {
    ...
    async handlePaymentSubmit(e) {
        ...
        try {
            // Show loading state
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Processing Payment...';
            ...
            this.showMessage(`Payment of $${amount.toFixed(2)} processed successfully!`, 'success');
    ...
    async handleTerminationSubmit(e) {
        ...
        try {
            // Show loading state
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Terminating Membership...';
            ...
            const saveResult = await this.dataManager.saveMembersToFile();
            if (!saveResult.success) {
                this.showMessage('Failed to terminate membership. Please try again.', 'error');
                return;
            }
