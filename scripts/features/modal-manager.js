...
class ModalManager {
	...
	async handleAddMember() {
		...
		try {
			...
			submitBtn.innerHTML =
				'<i class="fa-solid fa-spinner fa-spin mr-2"></i>Adding Member...';
			...
			const emailExists = this.dataManager.allData.some(
				(member) =>
			...
			this.showSuccessMessage(
				`Member ${memberData.name} has been added successfully!`,
			);
			this.onMemberAdded();
		} catch (error) {
